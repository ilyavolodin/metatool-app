...
export async function GET(req: NextRequest, context: { params: Promise<{ apiKey: string }> }) {
  ...
  try {
    ...
    logger.error(`Error in /api-key/${resolvedParams.apiKey}/mcp GET route:`, error);
...
export async function POST(req: NextRequest, context: { params: Promise<{ apiKey: string }> }) {
  const resolvedParams = await context.params;
  try {
    const apiKeyFromPath = resolvedParams.apiKey;
    if (!apiKeyFromPath) {
      ...
      const webAppTransport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: async (newSessionId) => {
          ...
          try {
            ...
            webAppTransport.send({jsonrpc: "2.0", id: "error", error: {code: -32000, message: "Failed to establish full session connection"}})
    ...
    logger.error(`Error in /api-key/${resolvedParams.apiKey}/mcp POST route:`, error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
